...
from src.utils.logging_utils import log_predictions
...
def main():
    ...
    log_file = log_manager.get_log_path('prediction')
    ...
    try:
        ...
        # Save predictions with log manager
        results_df = pd.DataFrame({
            'id': category_df.index,
            'predicted_price': predictions.flatten()
        })
        output_path = log_manager.get_log_path('prediction', os.path.basename(args.output_path))
